...
const sampleProtocolContract = "sample-contract-loan-v0";
...
function openLoan(chain: Chain, deployer: Account, callbackContract: string, loanParams: { vaultAmount: number, btcDeposit: number, liquidationRatio: number, liquidationFee: number } = { vaultAmount: 1000000, btcDeposit: 1, liquidationRatio: 14000, liquidationFee: 1000 }) {
  ...
  let loanBlock = chain.mineBlock([
    Tx.contractCall(callbackContract, "get-useraccount", [standardPrincipalCV(deployer.address)], deployer.address)
  ]);
  ...
  assertStringIncludes(createDLCInternalPrintEvent.contract_event.value, 'btc-deposit: u100000000, creator: STNHKEPYEPJ8ET55ZZ0M5A34J0R3N5FM2CMMMAZ6.sample-contract-loan-v0, emergency-refund-time: u10, event-source: "dlclink:create-dlc-internal:v0", liquidation-fee: u1000, liquidation-ratio: u14000, uuid: 0x66616b6575756964, vault-loan-amount: u1000000')
  ...
  assertStringIncludes(callbackPrintEvent.contract_event.value, 'user-id: u1, uuid: 0x66616b6575756964')
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block2 = chain.mineBlock([
      Tx.contractCall(contractPrincipal(deployer_2, sampleProtocolContract), "get-useraccount", [deployer.address], deployer.address)
    ]);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let block3 = chain.mineBlock([
      Tx.contractCall(contractPrincipal(deployer_2, sampleProtocolContract), "get-useraccount", [contractPrincipal(deployer_2, sampleProtocolContract)], deployer.address)
    ]);
    //The loan account in the sample protocl contact

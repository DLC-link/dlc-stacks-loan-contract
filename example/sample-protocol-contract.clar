...
(define-public (setup-user-contract (asset (buff 32)) (strike-price uint) (closing-time uint) (emergency-refund-time uint))
    (let 
      ...
      (begin
          ...
          (unwrap! (ok (as-contract (contract-call? .dlc-manager-pricefeed-v1-02 create-dlc asset strike-price closing-time emergency-refund-time target nonce))) err-cant-unwrap)
      )
    )
)
...
(define-public (close-dlc (nonce uint)) 
  (let (
    ...
    (begin 
      ...
      (unwrap! (ok (as-contract (contract-call? .dlc-manager-pricefeed-v1-02 close-dlc uuid))) err-cant-unwrap)
    )
  )
)
